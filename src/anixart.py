...
class AnixClient:
    ...
    def send_comment(
        ...
        if release_id:
            request_url = f"{self.api}/release/comment/add/{release_id}?token={self.token}"
        elif collection_id:
            request_url = f"{self.api}/collection/comment/add/{collection_id}?token={self.token}"
        return requests.post(
            request_url,
            json=data,
            headers=self.headers).json()
    ...
    def edit_comment(
        ...
        if release_comment_id:
            request_url = f"{self.api}/release/comment/edit/{release_comment_id}?token={self.token}"
        elif collection_comment_id:
            request_url = f"{self.api}/collection/comment/edit/{collection_comment_id}?token={self.token}"
        return requests.post(
            request_url,
            json=data,
            headers=self.headers).json()
    ...
    def bind_social_to_profile(
            ...
            google_idToken: str = None):
        data = {}
        if vk_accessToken:
            data["accessToken"] = vk_accessToken
            request_url = f"{self.api}/profile/preference/vk/bind?token={self.token}"
        elif google_idToken:
            data["idToken"] = google_idToken
            request_url = f"{self.api}/profile/preference/google/bind?token={self.token}"
        return requests.post(
            request_url,
            data=data,
            headers=self.headers).json()
